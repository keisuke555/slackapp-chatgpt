...
# bot-tokenとsocketModeHandlerを使ってアプリ初期化
app = App(
...
class SlackStreamingCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_new_token(self, token: str, **kwargs) -> None:
        ...
        if now - self.last_send_time > self.interval:
            ...
            # update_countが現在の更新間隔x10より多くなるたびに更新間隔をx2
            if self.update_count / 10 > self.interval:
                self.interval = self.interval * 2
    
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> Any:
        message_context = "Open APIで生成される情報は不正確または不適切な場合がありますが、当社の見解を述べるものではありません。"
        message_blocks = [
...
def handle_mention(event, say):
    ...
    # 投稿のキー(=Momentoのキー):初回=event["ts"],2回目以降=event["thread_ts"]
    id_ts = event["ts"]
